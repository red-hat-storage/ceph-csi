...
func (ns *NodeServer) blockNodeGetVolumeStats(
	...
	if !hasCachedChainSupport {
		...
		hasRequiredFeatures, err := rv.allImageInChainHasFeature(ctx, requiredFeatures)
		if err != nil {
			log.ErrorLog(ctx, "failed to check required features on image chain %s: %v, falling back to block metrics",
				rv, err)
